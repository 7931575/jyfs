...
public class ArchviesByNameSplit extends CRUD implements SplitDao<Employee> {
	...
	public int AllPageNumber(int size,String employeeName) {
		int num=0;
		String sql="select count(distinct e.employeeId) from employee e,archives a,department d where e.employeeId=a.employeeId and e.departmentId=d.departmentId and e.employeeName=?";
	...
	public List<Employee> ShowList(int size, int cruentPage,String name) {
		...
		String sql = "select distinct e.employeeId,e.employeeAccount,e.employeeName,d.departmentName from employee e,archives a,department d where e.employeeId=a.employeeId and e.departmentId=d.departmentId and employeeName=? limit ?,?";
		Object[] objects = { name,cruentPage, size };
		ResultSet rs = execute_Query(sql, objects);
		try {
			while (rs.next()) {
