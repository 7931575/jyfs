...
public class EmployeeDepartSplit extends CRUD implements SplitDao<Employee> {
	...
	public int AllPageNumber(int size,String departmentName) {
		int num = 0;
		String sql = "select count(*) from employee e inner join department d on e.departmentId=d.departmentId and d.departmentName like '%"+departmentName+"%'";
	...
	public List<Employee> ShowList(int size, int cruentPage,String departmentName) {
		List<Employee> list = new ArrayList<Employee>();
		String sql = "select e.*,d.departmentName from employee e inner join department d on e.departmentId=d.departmentId and d.departmentName like '%"+departmentName+"%' limit ?,? ";
		...
		try {
			while (rs.next()) {
				...
				Employee employee=new Employee(employeeId, employeeName, employeeAccount, department);
			 list.add(employee);
			}
		} catch (SQLException e) {
			e.printStackTrace();
