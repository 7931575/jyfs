...
public class ArchivesAllSplit extends CRUD   implements SplitDao<Employee> {
	...
	public int AllPageNumber(int size) {
		int num = 0;
		String sql = "select  count(distinct employeeId) from archives ";
	...
	public List<Employee> ShowList(int size, int cruentPage) {
		...
		String sql = "select distinct e.employeeId,e.employeeAccount,e.employeeName,d.departmentName from employee e,archives a,department d where e.employeeId=a.employeeId and e.departmentId=d.departmentId limit ?,?";
