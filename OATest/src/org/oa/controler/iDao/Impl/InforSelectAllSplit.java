...
public class InforSelectAllSplit extends CRUD implements SplitDao<Employee> {
	...
	public int AllPageNumber(int size,int departmentId,int grade) {
		int num=0;
		String sql="select count(*) from  employee e,job j, grade g where e.jobId=j.jobId and j.gradeId=g.gradeId and e.departmentId=? and g.grade<?";
	...
	public List<Employee> ShowList(int size, int cruentPage,int departmentId,int grade,String departmentName) {
		List<Employee> list=new ArrayList<Employee>();
		String sql="select e.jobId,employeeAccount,employeeName,e.employeeId from  employee e,job j, grade g where e.jobId=j.jobId and j.gradeId=g.gradeId and e.departmentId=? and g.grade<? limit ?,?";
