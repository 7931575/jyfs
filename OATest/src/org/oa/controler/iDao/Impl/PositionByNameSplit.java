...
public class PositionByNameSplit extends CRUD implements SplitDao<Employee> {
	...
	public int AllPageNumber(int size,String employeeName) {
		int num = 0;
		String sql = "select  count(distinct p.employeeId) from positions p inner join employee e on p.employeeId=e.employeeId and employeeName=?";
	...
	public List<Employee> ShowList(int size, int cruentPage,String employeeName) {
		...
		String sql = "select distinct e.employeeId,e.employeeAccount,d.departmentName from employee e,positions p,department d where e.employeeId=p.employeeId and e.departmentId=d.departmentId and e.employeeName=?  limit ?,?";
