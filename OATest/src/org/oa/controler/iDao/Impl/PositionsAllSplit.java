...
public class PositionsAllSplit extends CRUD implements SplitDao<Employee> {
	/**
	 * 获得总页数
	 **/
	@Override
	public int AllPageNumber(int size) {
		int num = 0;
		String sql = "select  count(distinct employeeId) from positions";
	...
	public List<Employee> ShowList(int size, int cruentPage) {
		...
		String sql = "select distinct e.employeeId,e.employeeAccount,e.employeeName,d.departmentName from employee e,positions p,department d where e.employeeId=p.employeeId and e.departmentId=d.departmentId limit ?,?";
