			...
			<c:if test="${requestScope.showList.size()>0}">
				...
				<table border="1" cellpadding="0" cellspacing="0">
					...
					<tr>
						...
						<td><a  href="jobServer?op=delete&jobId=${p.jobId}" onclick="return confirm('确定要删除吗')" >删除|</a><a href="jobServer?op=update&jobId=${p.jobId}">修改</a> </td>
					</tr>
					</c:forEach>
				...
				<c:if test="${requestScope.cruentPage!=page}">
					<a href="jobServer?op=show&cruentPage=${page}">${page}</a>
					</c:if>
					</c:forEach>
				</div>
				</c:if>
